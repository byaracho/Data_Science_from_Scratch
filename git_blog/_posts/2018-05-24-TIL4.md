...
$$ x^T A x > 0 $$<br />
(등호를 포함하면 positive semi-definite)<br />
<br />
...
선형대수학에서 놈 공간(normed space)은 벡터 공간의 각 원소의 놈을 통해 생기게 되는 벡터 공간을 의미한다.
<br />
##### 대각합(trace)
...
최소 자승 문제의 답은 $$x = ((A^TA)^{-1}A^T) b$$이며, 행렬 $$((A^TA)^{-1}A^T)$$를 행렬 A의 **의사 역행렬(pseudo inverse, $$A^{+}$$)**라고 한다. 일단 이 답만 기억해두고, 이는 나중에 행렬의 미분 공식을 사용하여 증명할 수 있다.<br />
<br />
